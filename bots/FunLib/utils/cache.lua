...
local GameStates = ____constants.GameStates
local CachedVarsCleanTime = ____constants.CachedVarsCleanTime
function ____exports.SetCachedVars(key, value)
    if not GameStates.cachedVars then
...
function ____exports.GetCachedVars(key, withinTime)
    if not GameStates.cachedVars or not GameStates.cachedVars[key] then
        return nil
    end
    if DotaTime() - GameStates.cachedVars[key .. "-Time"] <= withinTime then
